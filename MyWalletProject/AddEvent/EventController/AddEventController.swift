    ...
    func leftAction(){
        if tableViewController.tfDate.text! != "" && (tableViewController.tfMoney.text != "")  && (tableViewController.tfNameEvent.text != "") {
            ...
            if  arrayNameEvent.contains((tableViewController.tfNameEvent.text)!) == false {
                ...
                let event = ["dateEnd": tableViewController.tfDate.text!, "dateStart": setDate(), "goal": Int((tableViewController.tfMoney.text)!), "name": (tableViewController.tfNameEvent.text)! , "categoryid": tableViewController.event.categoryid, "spent": 0] as [String : Any]
                self.event.dateEnd = tableViewController.tfDate.text!
                self.event.goal = Int((tableViewController.tfMoney.text)!)
